...
// Or возвращает канал, который закрывается когда закрывается один из входящих каналов
		...
		go func(in <-chan interface{}) {
			for {
				...
				case <-res:
					return
				case _, ok := <-in:
					if !ok { // т.к. c буферизованный, то даже если несколько in каналов одновременно закроются, то горутина не зависнет
						c <- struct{}{}
						return
					}
				}
